...
class Transcricao:
    def __init__(self, path_video):
        ...
        self.client= OpenAI(api_key=settings.SK_OPENAI)
    ...
    def save_tempfile(self):
        audio_dir = Path(settings.BASE_DIR / "audio_file")
        audio_dir.mkdir(parents=True, exist_ok=True)  # Cria a pasta se não existir
...
def gerar_resumo(texto):
    ...
    response = client.completions.create(
        ...
        prompt= f"Resuma esse texto: {texto}"
    )
